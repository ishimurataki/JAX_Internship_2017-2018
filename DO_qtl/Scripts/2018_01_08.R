#############################################################
# Investigation of chromosome 11 peak on ins sec phenotypes
# January 8th, 2018
# - Taki Ishimura
#############################################################
library(qtl2)
...
ins_sec_chr11_peaks = ins_sec_peaks %>%
  filter(lod > 8) %>%
  filter(chr == 11)

# create parameters file for effect plots
output = as.character(ins_sec_chr11_peaks$analyte)
phenotypes = NA
...
analyses = cbind(output, phenotypes, transformation, covariates, ins_sec_chr11_peaks[,c("chr", "pos")], stringsAsFactors = FALSE)
...
covar <- cbind(pheno,
               ...
               DOwave5=(pheno$DOwave==5)*1)[,covar_transf]
...
saveRDS(out, "Derived_data/2018_01_08/scan1blups_chr11.rds")

# plot and save in pdf
pdf(file = "Plots/2018_01_08/chr11_qtlblups.pdf", width = 10, height = 6)
